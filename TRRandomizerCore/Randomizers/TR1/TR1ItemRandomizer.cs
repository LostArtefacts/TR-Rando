...
public class TR1ItemRandomizer : BaseTR1Randomizer
{
    ...
    private void FindUnarmedLevelPistols(TR1CombinedLevel level)
    {
        ...
        {
            List<TREntity> pistolEntities = level.Data.Entities.ToList().FindAll(e => TR1EntityUtilities.IsWeaponPickup((TR1Type)e.TypeID));
        ...
        for (int i = 0; i < level.Data.NumEntities; i++)
        {
            ...
            else if (TR1EntityUtilities.IsStandardPickupType(entityType))
            {
                ...
                if (newType == TR1Type.Pistols_S_P && (hasPistols || !level.Script.RemovesWeapons))
                {
                    ...
                    while (!TR1EntityUtilities.IsWeaponPickup(newType) || newType == TR1Type.Pistols_S_P);
                }
    ...
    public void RandomizeItemLocations(TR1CombinedLevel level)
    {
        ...
        for (int i = 0; i < level.Data.NumEntities; i++)
        {
            ...
            if (TR1EntityUtilities.IsStandardPickupType((TR1Type)entity.TypeID) && entity != _unarmedLevelPistols)
            {
    ...
    private List<Location> GetItemLocationPool(TR1CombinedLevel level)
    {
        ...
        {
            if (!TR1EntityUtilities.CanSharePickupSpace((TR1Type)entity.TypeID))
            {
    ...
    private void RandomizeSprites()
    {
        ...
        foreach (TR1Type type in TR1EntityUtilities.GetListOfKeyItemTypes())
        {
