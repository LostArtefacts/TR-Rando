...
public class TR3SecretRandomizer : BaseTR3Randomizer, ISecretRandomizer
{
    ...
    private TRSecretRoom<TR2Entity> MakePlaceholderRewardRoom(TR3CombinedLevel level)
    {
        ...
        if (ResourceExists(mappingPath))
        {
            ...
            {
                TR2Entity door = ItemFactory.CreateItem(level.Name, level.Data.Entities);
        ...
        foreach (Location location in locations)
        {
            if (_devRooms == null || _devRooms.Contains(location.Room))
            {
                ...
                if (PlaceSecret(level, secret, floorData))
                {
                    ...
                    TR2Entity entity = ItemFactory.CreateItem(level.Name, level.Data.Entities, secret.Location, true);
    ...
    private void RandomizeSecrets(TR3CombinedLevel level, List<TR3Type> pickupTypes, TRSecretRoom<TR2Entity> rewardRoom)
    {
        ...
        while (secret.SecretIndex < level.Script.NumSecrets)
        {
            ...
            {
                // This will either make a new entity or repurpose an old one
                TR2Entity entity = ItemFactory.CreateItem(level.Name, level.Data.Entities, secret.Location);
                entity.TypeID = (short)secret.PickupType;
