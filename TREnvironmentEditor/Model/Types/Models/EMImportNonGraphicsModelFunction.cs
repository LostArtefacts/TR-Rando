...
namespace TREnvironmentEditor.Model.Types
{
    public class EMImportNonGraphicsModelFunction : BaseEMFunction
    {
        ...
        public override void ApplyToLevel(TRLevel level)
        {
            ...
            RemapFaces(level.NumObjectTextures - 1, modelID => TRMeshUtilities.GetModelMeshes(level, (TREntities)modelID));
        }
        ...
        public override void ApplyToLevel(TR2Level level)
        {
            ...
            RemapFaces(level.NumObjectTextures - 1, modelID => TRMeshUtilities.GetModelMeshes(level, (TR2Entities)modelID));
        }
        ...
        public override void ApplyToLevel(TR3Level level)
        {
            ...
            RemapFaces(level.NumObjectTextures - 1, modelID => TRMeshUtilities.GetModelMeshes(level, (TR3Entities)modelID));
        }

        private void RemapFaces(uint maximumTexture, Func<short, TRMesh[]> meshAction)
        {
            foreach (EMMeshTextureData data in Data)
            {
                TRMesh[] meshes = meshAction.Invoke(data.ModelID);
                foreach (TRMesh mesh in meshes)
                {
                    foreach (TRFace3 face in mesh.ColouredTriangles)
                    {
                        face.Texture = SelectReplacementTexture(data, face.Texture, data.ColouredFace3, maximumTexture);
                    }
                    foreach (TRFace4 face in mesh.ColouredRectangles)
                    {
                        face.Texture = SelectReplacementTexture(data, face.Texture, data.ColouredFace4, maximumTexture);
                    }
                    foreach (TRFace3 face in mesh.TexturedTriangles)
                    {
                        face.Texture = SelectReplacementTexture(data, face.Texture, data.TexturedFace3, maximumTexture);
                    }
                    foreach (TRFace4 face in mesh.TexturedRectangles)
                    {
                        face.Texture = SelectReplacementTexture(data, face.Texture, data.TexturedFace4, maximumTexture);
                    }
                }
            }
        }
