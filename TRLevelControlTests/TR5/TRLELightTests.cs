...
[TestClass]
[TestCategory("Lights")]
public class TRLELightTests : TestBase
{
    [TestMethod]
    [Description("Test lights in a TRLE room that does not have fog bulbs.")]
    public void TestLightsNoFog()
    {
        ...
        void Test()
        {
            ...
            Assert.IsTrue(room.Lights[0] is TR5PointLight);
            Assert.IsTrue(room.Lights[1] is TR5ShadowLight);
            Assert.IsTrue(room.Lights[2] is TR5SpotLight);
            Assert.IsTrue(room.Lights[3] is TR5SunLight);
        }
    ...
    [TestMethod]
    [Description("Test fog bulbs in a TRLE room that has other lights.")]
    public void TestFogBulbsWithLights()
    {
        ...
        void Test()
        {
            ...
            Assert.AreEqual(4608, bulb.Position.X);
            Assert.AreEqual(-640, bulb.Position.Y);
            Assert.AreEqual(7680, bulb.Position.Z);
            Assert.AreEqual((byte)60, (byte)(bulb.Colour.R * 255));
            Assert.AreEqual((byte)63, (byte)(bulb.Colour.G * 255));
            Assert.AreEqual((byte)65, (byte)(bulb.Colour.B * 255));
            Assert.AreEqual(32767.5, bulb.Density);
            Assert.AreEqual(5120, bulb.Radius);
        }
    ...
    public void AddNonFogLight()
    {
        ...
        TR5Room room = level.Rooms[0];
        Assert.AreNotEqual(0, room.Lights.Count);
        Assert.IsFalse(room.Lights.Any(l => l is TR5FogBulb));
        ...
        TR5PointLight newLight = room.Lights[^1] as TR5PointLight;
    ...
    public void AddFogLight()
    {
        ...
        TR5FogBulb newBulb = room.Lights[^1] as TR5FogBulb;
    ...
    public void RemoveNonFogLight()
    {
        ...
        Assert.IsTrue(room.Lights[1] is TR5PointLight);
